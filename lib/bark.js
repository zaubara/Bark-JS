...
function Bark( input, optConfig ) {
  ...
  {
    this.parse(input);
    if(typeof optConfig == 'object') this.setConfig(optConfig);
  }
  else if(typeof input == 'object') this.setConfig(input);
...
Bark.prototype.AIparsers = {
  ...
  '10': function(inputVal, startPos){
    this.AIs['BATCH'] = inputVal.substr(startPos + 2, 6); // n..20, not 6
    return startPos + 2 + 6;
  },
